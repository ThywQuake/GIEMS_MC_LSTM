...
def collect(
    ...
    result_type: str = typer.Option(
        ...
        "--type",
        "-t",
        help="Type of results to collect: 'predictions' or 'evaluations'",
    ),
    output_file: str = typer.Option(
        None,
        "--output",
        "-o",
        help="Output file path. Defaults to '{pred_folder}/collected.npy' or '{eval_folder}/collected.npy'",
    ),
):
    """
    Gather results from all chunks after training/prediction completes.

    This function collects individual result files (predictions or evaluations)
    generated by parallel chunk processing and aggregates them into a single file.
    """
    import numpy as np
    from giems_lstm.config import Config
    ...
    if result_type == "predictions":
        source_folder = config.pred_folder
        default_output = source_folder / "collected.npy"
    elif result_type == "evaluations":
        source_folder = config.eval_folder
        default_output = source_folder / "collected.npy"
    else:
        print(f"Error: Unknown result type '{result_type}'. Use 'predictions' or 'evaluations'.")
        raise typer.Exit(code=1)

    output_path = Path(output_file) if output_file else default_output

    print(f"Collecting {result_type} from {source_folder}...")
    ...
    for lat_idx in range(mask.shape[0]):
        for lon_idx in range(mask.shape[1]):
            ...
            result_file = source_folder / f"{lat_idx}_{lon_idx}.npy"
            if result_file.exists():
                try:
                    data = np.load(result_file, allow_pickle=True).item()
                    collected_results[(lat_idx, lon_idx)] = data
                    files_found += 1
                except Exception as e:
    ...
    np.save(output_path, collected_results, allow_pickle=True)
    print(f"Collected results saved to {output_path}")

    # Print summary statistics for evaluations
    if result_type == "evaluations":
        _print_evaluation_summary(collected_results)


def _print_evaluation_summary(collected_results: dict):
    """
    Print summary statistics from collected evaluation results.
    """
    import numpy as np
    ...
    for (lat_idx, lon_idx), data in collected_results.items():
        ...
        if "R2" in train_metrics:
            train_r2_values.append(train_metrics["R2"])
        if "R2" in test_metrics:
            test_r2_values.append(test_metrics["R2"])
        if "RMSE" in train_metrics:
            train_rmse_values.append(train_metrics["RMSE"])
        if "RMSE" in test_metrics:
            test_rmse_values.append(test_metrics["RMSE"])

    print("\n=== Evaluation Summary ===")
    if train_r2_values:
        print(f"Train R2: mean={np.mean(train_r2_values):.4f}, std={np.std(train_r2_values):.4f}")
    if test_r2_values:
        print(f"Test R2:  mean={np.mean(test_r2_values):.4f}, std={np.std(test_r2_values):.4f}")
    if train_rmse_values:
        print(f"Train RMSE: mean={np.mean(train_rmse_values):.4f}, std={np.std(train_rmse_values):.4f}")
    if test_rmse_values:
        print(f"Test RMSE:  mean={np.mean(test_rmse_values):.4f}, std={np.std(test_rmse_values):.4f}")
